...
async function createHandler(
  ...
  if (d.externalTrackingNo && d.shippingPartner && d.shippingPartner !== 'internal') {
    ...
    if (existingShipment) {
      return NextResponse.json<ApiResponse<never>>(
        {
          success: false,
          error: `Tracking ${d.externalTrackingNo} is already linked to shipment ${existingShipment.trackingNo}`,
        },
        { status: 409 }
      )
    }
  ...
  // Generate tracking number with retry on collision (count is non-atomic under concurrency)
  let shipment: { id: string; trackingNo: string } | null = null
  let attempts = 0
  while (!shipment && attempts < 5) {
    const todayStart = new Date(); todayStart.setHours(0, 0, 0, 0)
    const seqCount = await prisma.shipment.count({ where: { createdAt: { gte: todayStart } } })
    const trackingNo = generateTrackingNo(seqCount + 1 + attempts)

    try {
      // Create shipment + initial event + COD in a transaction
      shipment = await prisma.$transaction(async (tx: any) => {
        ...
        return { id: s.id, trackingNo: s.trackingNo }
      })
    } catch (e: any) {
      // P2002 = unique constraint violation on trackingNo — retry with next sequence
      if (e?.code === 'P2002' && e?.meta?.target?.includes?.('trackingNo')) {
        attempts++
        continue
      }
      throw e
  ...
  await writeAuditLog(user.id, 'SHIPMENT_CREATED', 'Shipment', shipment.id, null, { trackingNo: shipment.trackingNo, price: finalPrice })
